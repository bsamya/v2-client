...
const rules = {
  ...
  documents: {
    invoice: {
      required: helpers.withMessage(
        'At least one invoice must be uploaded',
        (v: string[]) => v.length > 0
      )
    },
    manufacturingStatement: {
      required: helpers.withMessage(
        'Manufacturing Statement must be uploaded',
        (v: string[]) => v.length > 0
      )
    }
  }
...
const stepperErrors = computed(() => {
  ...
  if (error.value['documents.invoice'] || error.value['documents.manufacturingStatement']) e.step4 = false
  return e
})
...
<template>
  <div>
    <v-stepper v-model="step"
               non-linear
               alt-labels>
      <v-toolbar title="CFS Application"
                 flat>
